...
MC_VERSIONS_REGEX = re.compile(r"^[0-9.,\-\s]+$")
...
def looks_like_mc_versions(value: str) -> bool:
    if not value.strip():
        return False
    return bool(MC_VERSIONS_REGEX.match(value))
...
def validate_plugin_fields(entry: dict[str, Any], name: str) -> tuple[list[str], list[str]]:
    ...
    elif not looks_like_mc_versions(entry["mc_versions"]):
        errors.append(error(f"`mc_versions` contains invalid characters in plugin '{name}'."))
